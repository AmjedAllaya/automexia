...
/// Scalar path for wasm and Miri, where the C++-backed `simdutf` is unavailable.
/// std determines the valid prefix; `compute_error_len` normalizes the failure shape to the SIMD contract.
...
pub fn validate(bytes: &[u8]) -> Result<&str, Utf8Error> {
    std::str::from_utf8(bytes).map_err(|err| {
        ...
        Utf8Error {
            valid_up_to,
            error_len: compute_error_len(bytes, valid_up_to),
        }
    })
}
