...
pub enum SettingKind {
    ...
    /// Finite continuous values; step is a UI increment, not a value lattice.
    ContinuousNumber {
        min: f64,
        max: f64,
        step: f64,
...
pub struct AppearanceValues {
    pub font_size: f64,
    pub configured_font_size: f64,
    pub font_min: f64,
    pub font_max: f64,
    pub font_origin: ValueOrigin,
    pub theme: AppearanceChoice,
}

/// Shared metadata only; the application owns config, effects and persistence.
pub fn appearance_descriptors(values: AppearanceValues) -> Vec<SettingDescriptor> {
    let valid_font = |value: f64| {
        value.is_finite() && (values.font_min..=values.font_max).contains(&value)
    };
    let font_supported =
        valid_font(values.font_size) && valid_font(values.configured_font_size);
    let font = SettingDescriptor {
        id: SettingId(FONT_SIZE.into()),
        owner: SettingOwner::Core,
        ...
        value: if font_supported {
            SettingValue::Number(values.font_size)
        } else {
            SettingValue::Action
        },
        default: if font_supported {
            SettingValue::Number(values.configured_font_size)
        } else {
            SettingValue::Action
        },
        origin: values.font_origin,
    ...
    let theme = SettingDescriptor {
        id: SettingId(APPEARANCE_THEME.into()),
        owner: SettingOwner::Core,
        section: Section::Appearance,
        ...
        kind: SettingKind::Choice {
            options: [
                (AppearanceChoice::Configuration, "Use configuration"),
        ...
        value: SettingValue::Choice(values.theme.value().into()),
        default: SettingValue::Choice(AppearanceChoice::Configuration.value().into()),
        origin: if values.theme == AppearanceChoice::Configuration {
...
fn value_matches(kind: &SettingKind, value: &SettingValue) -> bool {
    match (kind, value) {
        ...
        (SettingKind::ContinuousNumber { min, max, .. }, SettingValue::Number(value)) => {
