...
#[test]
fn appearance_continuous_numbers_preserve_fractional_values_without_weakening_discrete_steps(
) {
    ...
    row.value = SettingValue::Number(21.5);
    row.default = SettingValue::Number(18.25);
    let catalog = Catalog::new(7, vec![row.clone()]).unwrap();
    for value in [6.0, 6.125, 21.5, 22.5, 99.875, 100.0] {
        assert!(
            catalog
                .validate_edit(&edit(
                    INLINE_TABLES,
    ...
    for value in [f64::NAN, f64::INFINITY, f64::NEG_INFINITY, 5.999, 100.001] {
        assert_eq!(
            catalog.validate_edit(&edit(
                INLINE_TABLES,
                Change::Set(SettingValue::Number(value))
    ...
    assert_eq!(
        Catalog::new(7, vec![row]).unwrap_err(),
        SettingsError::InvalidValue
    );
}

#[test]
fn appearance_continuous_number_metadata_rejects_invalid_or_unbounded_increments() {
    for (min, max, step) in [
        (6.0, 100.0, 0.0),
        (6.0, 100.0, -1.0),
        (100.0, 6.0, 1.0),
        (6.0, 100.0, f64::INFINITY),
        (6.0, f64::INFINITY, 1.0),
        (6.0, 100.0, 0.00000001),
    ] {
        let mut row = core().entries()[0].clone();
        row.kind = SettingKind::ContinuousNumber { min, max, step };
        row.value = SettingValue::Number(18.0);
