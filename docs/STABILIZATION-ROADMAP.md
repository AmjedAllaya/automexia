...
| CP5 Shell Completion and Suggestions | CP5.0 research fully done with CP1 retained; ADR 0025 and its machine threat contract accepted; CP2/CP3 for action candidates; cached-public CP4/D6 context only | CP5.1-CP5.3 are fully done at their source/local model boundaries. CP5.4 is source-integrated but unpublished. CP5.5 is request-only inert scaffolding, and CP5.6 activation/release evidence remains partial or external. | No grid/history-file/remote-output inference, network/auth/secret work, or per-keystroke process. Preview remains disabled until native insertion, IME/grapheme, accessibility, responsive layout, native shell/OS, cancellation, latency, leak, 30-day baseline, signed package, disable, uninstall, and measurable-benefit proof passes. Not a v0.5.0 blocker. |
...
| CP5.1 bridge — **Fully done at source boundary / partially done for release** | Schema-1 bidirectional models/codecs, strict limits, route/capability/generation/span revalidation, restrictive Windows/Unix endpoint sources, joined latest-only service, cleanup, property/fuzz and native Windows tests | Native Linux `SO_PEERCRED`, macOS `getpeereid`, WSL relay, crash/restart/sleep, and sustained endpoint-churn evidence remains |
...
| CP5.4 UI — **Partially done** | Renderer-neutral pane-owned listbox, branded draw-only rows, source/freshness/risk text, matched graphemes, responsive cursor/exclusion placement, compact fallback, reduced-motion/accessibility semantics and lifecycle invalidation | Runtime publication, native GPU screenshots, high-contrast/IME/pointer/keyboard automation, and controlled screen-reader sessions remain |
| CP5.5 shells — **Partially done** | Inert version/preview/collision/disable guarded request scaffolds for PowerShell 7, Bash 5, Zsh 5.8, and Fish 3.6; truthful CP1/CMD/unsupported fallback | Signed helper, response/replacement adapters, WSL relay, Fish Unicode path, native insertion/profile preservation, packages, and activation remain not done |
| CP5.6 release — **Partially done** | Disabled-by-default composition, runtime kill/reset/disable/uninstall, LKG health, policy mutations and local lifecycle checks | Three-OS native CI, controlled NVDA/Narrator/VoiceOver/Orca, signed package/rollback, 1,000 real endpoint cycles, leak/storm campaigns and a 30-day named-hardware baseline remain |
