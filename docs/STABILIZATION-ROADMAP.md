...
| CP5 Shell Completion and Suggestions | CP5.0 research fully done with CP1 retained; CP5.1+ requires a separately reviewed bridge ADR/threat fixture; CP2/CP3 for action candidates; CP4/D6 for provider context | CP5.0 shell/API, prototype, dependency, benchmark, privacy, and fallback decision is complete without activation. CP5.1 authenticated local bridge through CP5.6 preview/rollback remain not done. | No grid/history-file/remote-output inference, network/auth/secret work, or per-keystroke process. Any future P2 must prove protocol/fuzz/security, exact insertion, IME/grapheme, accessibility, responsive layout, native shell/OS, cancellation, latency, leaks, 30-day baseline, disable, uninstall, and measurable benefit. Not a v0.5.0 blocker. |
