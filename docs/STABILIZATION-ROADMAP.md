...
| Terminal image protocols and quick look | Missing local filename UX; iTerm2 decode was less bounded than Kitty/Sixel | Sixel, Kitty placements/placeholders, and iTerm2 inline images are preserved. Explicit modifier-hover and selected-path preview now use a bounded route/generation worker, local/regular-file policy, WSL validation, responsive overlay, and tested decoder limits. | Controlled native visual/protocol evidence on Windows, Linux, and macOS; retain multiplexer/client compatibility evidence. |
...
| Local image quick look | Complete locally at the source boundary | Alt/Cmd modifier dwell and selected-path/palette actions resolve only local raster candidates, decode/downscale off-thread with fixed bounds, scope results to route/generation, evict on dismissal, and hide at unusable geometry. Existing terminal image protocols remain independent. |
