...
| CP2 typed Quick Actions | CP0; **CP2.0 pure model complete**; D3 not required for insert/copy | CP2.1+: bounded versioned store; atomic user-only writes; last-known-good reload; exact-file watcher; layered scopes; CRUD/import/export; placeholder review; accessible search; insert/copy only. | CP2.0: bounded parser/model plus 11-case hostile corpus and capability ratchet pass. CP2.1/2 still require persistence/recovery/concurrent-window/isolation, Unicode, leak, accessibility, and Windows/Linux/macOS evidence. |
...
CP2.0 is complete at the pure model boundary; CP2.1 persistence is next.
Stable release claims still require the pushed hosted Windows,
...
| Bounded validation | Top-level ceilings were frozen but nested tags and mutating acknowledgement were implicit | 1 MiB pre-decode limit; 1,024 actions; 256 enabled aliases; 64 arguments/tags; 32 placeholders; 4 KiB strings; portable IDs; duplicate/reference/shell/mode/risk/secret/raw-insert/control/bidi rejection | CP2.1 must enforce the same limits before and during persistence/recovery |
...
| Architecture | CP0 rejected all Quick Action runtime vocabulary | Exact three-file allowlist; 12-class capability mutation matrix rejects filesystem/process/network/environment/async/UI/PTY/unsafe drift; all other runtime files remain denied | Store, watcher, UI, projection, provider, and execution authority require later phase gates |
| Status truth | CP2/CP3 documented only as planned | Machine contract says `CP2.0-model-only`; feature catalog and canonical spec distinguish implemented groundwork from unshipped persistence, aliases, UI, and exact launch | CP2.1 is the next implementation phase |
