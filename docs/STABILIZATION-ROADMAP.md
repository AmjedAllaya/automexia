...
| Rendered-frame evidence | Partial | Structured state and native Windows capture remain. `cargo xtask visual-diff` bounds regular-file input, 8K-equivalent dimensions, 40 million pixels, decoded allocation, reports, and outputs; the canonical policy rejects one changed channel in one unmasked pixel with zero changed-pixel tolerance and atomically emits heatmap/JSON artifacts. The S1 contract enumerates five 8,352-frame matrices across high-contrast, 100–400% scale, and enabled/reduced motion. | Record and approve the exact expected/actual matrices, Linux/macOS native frames, and human visual review across viewport, theme, font, DPI, and motion states. |
