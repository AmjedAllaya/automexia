...
| Terminal image protocols and quick look | Missing local filename UX; iTerm2 decode was less bounded than Kitty/Sixel | Sixel, Kitty placements/placeholders, and iTerm2 inline images are preserved. Explicit modifier-hover and selected-path preview now use a 16-owner latest-generation queue, strict predecode dimension/magic/file-version gates, a 16-entry/32 MiB LRU, zero-copy stable renderer identity, WSL validation, responsive overlay, and tested decoder limits. | Controlled native visual/protocol evidence on Windows, Linux, and macOS; retain multiplexer/client compatibility evidence. |
...
| Local image quick look | Complete locally at the source boundary | Alt/Cmd modifier dwell and selected-path/palette actions resolve only local raster candidates, decode/downscale off-thread with preallocation bounds, coalesce work per window, reuse only file-version-matched thumbnails within 32 MiB, share pixels/stable GPU identity, remove active entries on dismissal, and hide at unusable geometry. Existing terminal image protocols remain independent. |
