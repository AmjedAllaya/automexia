...
| **Partially done** | M6/F6 | Schema-2 reviewed library edits/migration/transfer, typed recipe review/lifecycle/no-hooks, narrow remote initialization, declarative workspace restore, and armed broadcast are source-complete locally. Proposed ADR 0023 acceptance, product wiring/execution, and controlled native/resource/accessibility evidence remain. |
...
| **Partially done** | D6.1/M8 | AWS is source-complete and nonactivated: independent bounded public-profile parsing, exact IAM Identity Center/STS operations, strict public STS evidence, exact SSM review plan, EKS dry-run intent, and focused security/argv/version/redaction tests pass locally. D3 activation, M11 ingestion, and real native/provider evidence remain. |
| **Partially done** | D6.2/M9 | Azure is source-complete and nonactivated: bounded public account JSON, exact tenant/login/account operations, AAD-only Bastion, opaque transient AKS intent, and focused security/argv/isolation tests pass locally. D3 activation, M11 ingestion, and real native/provider evidence remain. |
| **Not done** | D6.3-D6.5 | Google Cloud, Kubernetes/OpenShift, Teleport, and OpenBao adapters, product login flows, and real native/provider evidence remain planned. OpenBao implementation is blocked on proposed ADR 0024 acceptance. |
