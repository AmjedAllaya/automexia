...
|---|---|---|
| **Fully done (source and repository setup)** | Exact x64/Arm64 DEB, RPM, and tar allowlist; bundle/manifest/checksum policy; credential-free rehearsal trigger and fail-closed isolation; bounded free-runner quality policy; complete release/per-asset attestation gates; mutation tests; passive public archive; immutable releases; active signed-commit default-branch and non-rewritable `v*` tag rulesets; merged inactive website integration; live fail-closed routes | Preserve these contracts and rerun them for every release change. |
| **Partially done (external release evidence required)** | Successful credential-free native rehearsal; real minisign bundle; scoped GitHub App publication; immutable post-upload and GitHub attestation evidence; and website activation handoff | Pass the corrected hosted x64/Arm64 rehearsal, add an independent reviewer, configure the real signing key and App, merge the reviewed archive policy change, then pass the exact reviewed release jobs. |
