...
| **Partially done** | Official signed Linux Early Access publication | Native x64/Arm64 packages, quality and signing passed in run `34028707592`; both existing credentials and the App's exact repository/permission scope were verified. The read-only governance API fix passes live and regression checks but requires a new guarded post-merge publication. Offline backup is a separate owner responsibility. ADR 0040 accepts owner-only authorization. |
