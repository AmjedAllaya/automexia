...
| **External prerequisite** | Official signed Linux Early Access publication | All four credential settings are registered. Key/App validity, offline backup and the guarded post-merge result remain required. ADR 0040 explicitly accepts owner-only authorization; a second reviewer is not required for this channel. |
