...
8. Inspector and complete-top-level-tab undo/redo follow accepted redaction and
   PTY lifecycle decisions; broader topology history remains outside scope.
...
## Implementation result ledger — 2026-08-23
...
| G5 tooling/release | **Partially done** | Pre-GUI CLI/explain/JSON; bounded migration; xtask generate/verify/test; generated references; properties; two fuzz targets; Windows Criterion benchmarks | Native three-platform layout/visual/AT/resource matrix, fixed fuzz run, QA bundle, and activated 30-day baseline |
| G6 lifecycle | **Partially done** | ADRs 0027/0028; redacted inspector; memory-only, count/time/history-bounded parked-PTY undo/redo for a complete closed top-level tab | Individual split/local-tab/native-window history and native lifecycle proof |
...
- G6 parks only complete closed top-level tabs. Their PTYs remain independently
  owned and count/time/history bounded; restore failure destroys the entry safely
...
doctests, full repository QA, and `cargo ready`. Windows-native evidence can be
recorded on this host; Linux/BSD, macOS, real keyboard layouts, controlled
assistive technology, and named-hardware 30-day benchmarks remain explicit
release prerequisites unless those exact environments run.
