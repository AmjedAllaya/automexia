...
| Connection Hub | Compact shared-brand card, selected section, visible `C`/`W`/`P`/`L`/`F` keycaps, labeled actions, one local-only safety statement, and concise loading/error states | 40-pixel section/action/close targets; C/W/P switch top-level lists, L opens host review, F opens the same exact-file picker as the pointer action, and Search/nested reviews retain their own text; terminal input behind the scrim is inert | Native picker pixels and Narrator/NVDA, VoiceOver, and Orca evidence remain release-gated |
