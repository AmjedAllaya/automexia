...
| Command palette | Search input, selected command, visible shortcut, category label, and a persistent vertical overflow indicator whose active state does not rely on color alone | Open, filter, move, activate, and dismiss without a pointer; wheel and trackpad motion scroll the owned result window while keeping the selected row visible | Platform role and scroll-position announcements require the v0.5 adapter |
...
| Connection Hub | Spacious 840×500 shared-brand setup card, focused 840×420 direct-entry card, selected section, visible `C`/`W`/`P`/`L`/`F` keycaps, persistent field labels, one local-only safety statement, and concise loading/error states | 40-pixel section/close targets, 44-pixel setup actions, 40-pixel direct actions on usable viewports, responsive field stacking, and top-right nested cancellation; C/W/P switch top-level lists, L opens host review, F opens the same exact-file picker as the pointer action, and Search/nested reviews retain their own text; terminal input behind the scrim is inert | Native picker pixels and Narrator/NVDA, VoiceOver, and Orca evidence remain release-gated |
...
| Completed command output | Command-agnostic persistent tint, adaptive 6-10 pixel gutter, end rule, terminal-owned local completion datetime, known-status icon/duration or explicit neutral treatment, one 540 ms lightening, and stable pane-local boundary identity through viewport/full source-prompt eviction | Passive feedback only; shell input, cursor, selection, copy, search, and history keep their existing owners | Datetime remains readable text and compactly degrades before disappearing; native screen readers consume terminal text, but the visual result grouping is not yet exposed as a native region |
...
9.9% tint, end rule, whitespace, local completion datetime, and known-status
icon plus elapsed time retain the boundary after the temporary lightening ends.
Full ISO local datetime drops duration first and then compacts to month-day plus
hour-minute when pane width is constrained; it never clips into prompt context.
The cue changes opacity only;
