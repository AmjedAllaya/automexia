...
| Performance safeguards | Existing latency/resource ceilings remain. S2 now has strict bounded Criterion/native-memory normalization, exact run composition, a digest-bound 30-90-day builder, exact expiring waivers, 90-day nightly retention, and tagged-release enforcement at 5% latency/10% memory. Linux latency and Windows complete-run jobs are distinct; unknown metrics, runner drift, duplicate/non-finite/oversized evidence, and inactive baselines fail closed. The checked-in baseline is still collecting, so these source gates do not claim the missing 30 controlled days. |
| Phase 0 assurance tooling | Pinned Nextest/JUnit/doctests, bounded redacted QA, viewport/DPI properties, Loom, and native stress remain. S1 adds a 512-case independent resize-queue model, bounded generation-aware atomic snapshot replacement, and an 8K/40-million-pixel/32-mask visual comparator with reviewed 2-channel/0.1% tolerance and atomic path-free evidence. Controlled native/golden/human review remains separate. |
...
| Phase 2 exact-launch preparation | ADR 0012 is accepted by the project owner. The D0/D3/M5 schema 5 preserves immutable schemas 1-4 and freezes manual-shell/missing-client behavior, package identity, exact M3-M5 direct/routed/tunnel grammar, trust/lifecycle, grants/audits/defaults, nine boundaries, fixed Windows/macOS/Linux and disabled-WSL resolution, and 23 source/build/fixture/security/resource scenarios. The production broker remains hard disabled and the package unverified. One application runner owns exact requests, 50 active operations, 256 redacted audits/receipts/reconnect candidates, current executable binding, truthful outcomes, cancellation, shutdown, and ContextManager-only PTY/route publication. Typed tunnels add loopback defaults, strong Allow-once review, compact accessible state, and bounded owner cleanup without opening application sockets. The synthetic manifest only validates the contract. ADR 0003 approvals/enforcement, attestation/revocation, actual status/SSH execution, real Windows/macOS/Linux OpenSSH/cleanup/resources/accessibility, and production activation remain open. |
...
| v0.4 S1 structured/rendered visuals | **Source fully implemented.** Exact `s1-standard-v1` test-only fixture state freezes public content, clock, and motion; the bounded comparator remains authoritative; and the versioned policy requires 560 theme/scale/viewport/surface cases with independent HTTPS human review. | Execute and approve the policy matrix on Windows, Linux X11/Wayland, and macOS Intel/Apple Silicon hosts. Retain native frames and reviewer decisions for the exact protected commit. |
...
| v0.4 S1/S2 performance/resources | Strict source ratchet, native memory composition, 90-day retention, reviewed baseline builder, and fail-closed release job are implemented; status is collecting | Run and review 30 consecutive complete same-runner days; elevated WPR remains separate. Compile-only/local results do not count. |
| v0.4 S1 AppVerifier/native matrix | **Source fully implemented.** Native Windows WGPU/CPU GUI/resource storms pass locally; the exact-target wrapper separates Basics from bounded low-resource injection, rejects verifier errors/stops and oversized logs, and emits distinct redacted evidence. The strict 24-suite validator binds evidence to policy, protected commit, environment, freshness, and clean tracked state. | Run elevated AppVerifier and WPR plus named Intel/AMD/NVIDIA/RDP, Linux X11/Wayland, and macOS Intel/Apple Silicon suites; retain reviewed driver/adapter evidence. |
