...
| Phase 2 exact-launch preparation | ADR 0012 is accepted by the project owner. The D0/D3/M5 schema 5 preserves immutable schemas 1-4 and freezes manual-shell/missing-client behavior, package identity, exact M3-M5 direct/routed/tunnel grammar, trust/lifecycle, grants/audits/defaults, nine boundaries, fixed Windows/macOS/Linux and disabled-WSL resolution, and 23 source/build/fixture/security/resource scenarios. The production broker remains hard disabled and the package unverified. One application runner owns bounded off-hot-path current-executable review, exact requests, 50 active operations, 256 redacted audits/receipts/reconnect candidates, guarded identity, truthful outcomes, cancellation, process-group/Job Object teardown, bounded PTY-worker joining, shutdown, and ContextManager-only PTY/route publication. Typed tunnels add loopback defaults, strong Allow-once review, compact accessible state, and bounded owner cleanup without opening application sockets. The synthetic manifest only validates the contract. ADR 0003 approvals/enforcement, attestation/revocation, actual status/SSH execution, real Windows/macOS/Linux OpenSSH descendant/resources/accessibility proof, and production activation remain open. |
...
| v0.4 S1/S2 performance/resources | S2 source/automation is fully implemented with strict sample quality, clean exact-source/operator binding, native memory composition, independent review, protected activation, 90-day retention, and fail-closed release enforcement; status is collecting | Provision the declared runner/variables, run and independently review 30 consecutive complete same-runner days, activate through `stable-release`, and retain the exact summary; elevated WPR remains separate. Compile-only/local results do not count. |
