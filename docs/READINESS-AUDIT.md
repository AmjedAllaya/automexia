...
| Phase 2 exact-launch preparation | ADR 0012 is accepted by the project owner. The schema-2 D0/D3 contract preserves immutable history and freezes manual-shell/missing-client behavior, package identity/version/contract/verification, grants/audits/defaults, nine trust boundaries, fixed Windows/macOS/Linux and disabled-WSL resolution, and nineteen native scenarios. The production-compiled broker remains hard disabled and the linked package remains unverified. One application runner now owns exact typed requests, 50 active operations, 256 redacted audits, bounded public environment, trusted cwd, cancellation, and shutdown. The guarded PTY seam consumes a re-compared exact executable; ContextManager publishes one route only after PTY creation. The actionable review provides allow-once/session/deny pointer, keyboard, focus, accessibility, and fixed recovery states. ADR 0003 exact-head approvals/server enforcement, real loader attestation/revocation, native graceful/forced cleanup, OpenSSH fixture execution, D4-to-D5 activation, and controlled process/PTY/renderer/accessibility/resource gates remain open. |
