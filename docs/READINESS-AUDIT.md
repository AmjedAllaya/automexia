...
| Phase 2 exact-launch preparation | The local schema-1 D0/D3 contract freezes exact package ID/publisher/digest/workspace-version/contract and reviewed-or-signed proof, grant/audit/default/authority fields, fixed Windows/macOS/Linux and disabled-WSL resolution, and nineteen four-platform native scenarios. The test-only broker enforces that package policy plus exact expiring capability/decision/session/capsule scope, replay-safe leases, fail-closed resolution and file revalidation, literal bounded argv, environment/secret isolation, cwd fallback, revocation, bounded lifecycle, and redacted audit. The production graph exposes no managed process/PTY/network path. Protected ADR 0012 acceptance, real loader attestation/revocation binding, capability UI, atomic native launch, execution of the native matrix, D4-to-D5 activation, and controlled process/PTY/renderer performance/leak gates remain open. |
