...
| Image previews | Sixel, Kitty Graphics placements/placeholders, and iTerm2 inline images remain renderer-integrated. Local printed/selected raster paths now have explicit Alt/Cmd hover and Ctrl/Cmd+Alt+I quick look with predecode security gates, a 16-owner latest-generation queue, 16-entry/32 MiB file-version cache, zero-copy stable renderer identity, WSL validation, responsive geometry, palette discovery, and iTerm2 decode hardening. Focused source tests pass; controlled cross-platform visual/client evidence remains a release gate. |
...
| Performance safeguards | History interaction and resize delivery are fixed and measured on Windows. No-damage snapshots, prompt repaint, cache access, worker submission, parser/rebuild/reflow, and bounded queues/caches have focused tests or Criterion cases. Nightly keeps compile-only benchmark validation separate and now defines an opt-in named-runner job that executes and bundles Criterion evidence. The isolated image benchmark was executed locally on Windows: cold 1600x1000 decode/resize measured 24.683 ms and warm validated reuse 48.984 us (about 504x). This is evidence for the implementation choice, not the controlled 30-day baseline, so cross-host performance enforcement remains external. |
