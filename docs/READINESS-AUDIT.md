...
| Image previews | Sixel, Kitty Graphics placements/placeholders, and iTerm2 inline images remain renderer-integrated. Local printed/typed raster paths now support IO-free plain-hover discovery, 100 ms bounded background submission, click-to-pin, visible-path arrow browsing, Escape dismissal, selection/palette access, complete mouse-event ownership, predecode gates, a 16-owner latest-generation queue, 16-entry/32 MiB file-version cache, shared stable renderer identity, WSL validation, responsive geometry, and iTerm2 hardening. Decoder/token/cache logic is isolated in renderer-free `automexia-image`. Focused source tests, a DPI-correct real-pointer Windows `ls`/hover/click/arrow/Escape/GPU-overlay path, and a 544,609-execution explicit-nightly WSL fuzz campaign pass; controlled Linux/macOS visual/client evidence remains a release gate. |
...
| Performance safeguards | History interaction and resize delivery are fixed and measured on Windows. No-damage snapshots, prompt repaint, cache access, worker submission, parser/rebuild/reflow, and bounded queues/caches have focused tests or Criterion cases. Image hit testing is IO-free, hover is 100 ms debounced, warm previews reuse stable shared pixels, and native WGPU/CPU tests reject black or card-obscured preview bodies. Nightly keeps compile-only benchmark validation separate and defines an opt-in named-runner job that executes and bundles Criterion evidence. The isolated image benchmark measured cold 1600x1000 decode/resize at 24.683 ms and warm validated reuse at 48.984 us (about 504x). This is evidence for the implementation choice, not the controlled 30-day baseline, so cross-host performance enforcement remains external. |
