...
| Phase 2 exact-launch preparation | ADR 0012 is accepted by the project owner. The schema-3 D0/D3 contract preserves immutable schema-1/schema-2 history and freezes manual-shell/missing-client behavior, package identity/version/contract/verification, grants/audits/defaults, nine trust boundaries, fixed Windows/macOS/Linux and disabled-WSL resolution, and nineteen native scenarios. The production-compiled broker remains hard disabled and the linked package remains unverified. One application runner owns exact managed requests, 50 active operations, 256 redacted audits/receipts/reconnect candidates, bounded public environment, trusted cwd, actual child outcomes, cancellation, and shutdown. The guarded PTY seam consumes a re-compared exact executable; ContextManager publishes one route only after PTY creation; fixed notifications, a private 256-record/2-MiB atomic receipt store, and stale-source reconnect preparation are source-complete. The actionable review provides allow-once/session/deny pointer, keyboard, focus, accessibility, and fixed recovery states. ADR 0003 exact-head approvals/server enforcement, real loader attestation/revocation, native graceful/forced cleanup, OpenSSH fixture execution, D4-to-D5 activation, and controlled process/PTY/renderer/accessibility/resource gates remain open. |
