...
| Phase 2 exact-launch preparation | A test-only D3 broker model now enforces an exact first-party principal, typed capability/decision/session/capsule scope, fixed absolute OpenSSH locations, native file identity and revalidation, literal bounded argv, zero extension access to inherited environment/secrets, safe cwd fallback, generation leases, revocation, and redacted audit records. The production module graph excludes the broker and no managed process/PTY/network path is enabled. ADR acceptance, capability UI, real native lifecycle evidence, D4 SSH security, and controlled performance/leak gates remain open. |
