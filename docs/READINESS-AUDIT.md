...
| Shell integration | Every PowerShell source parses; the Windows contract covers isolated automatic install/no-op/repair passes, repeated sourcing, PowerShell plus an in-process native CMD prompt, identity, cloning, accepted-input ownership, and icon-aware listing smoke while preserving explicit `cmd /c`, built-in `dir`, and the user's PSReadLine history predicate. A TTY-only compatibility layer gives Ubuntu/WSL eza 0.18.x the same composite folder badges without changing redirected output. Bash/Zsh syntax, isolated automatic install/no-op/repair passes, live eza output, ShellCheck, and integration jobs are part of the Unix local/CI gate. The Unix validator now uses a BSD/macOS-compatible `mktemp` template and is mandatory on native Linux and macOS; a font parser proves every category glyph exists in the bundled Symbols Nerd Font. |
...
| Feature and platform assurance traceability | A schema-validated ledger covers all workspace members and required shell, package, workflow, and automation surfaces across correctness, security, performance, resource lifetime, storage hygiene, resilience, accessibility, visual quality, Windows, Linux, and macOS. It owns all five Criterion and seven fuzz targets. Mutation suites fail when native hosts, shells, X11/Wayland variants, alternate architectures, package validators, benchmark/fuzz ownership, or evidence paths/jobs/headings disappear. External Linux/macOS GPU/PTY/accessibility and representative-not-universal Linux distribution coverage remain explicit rather than reported as passed. |
...
| Phase 2 exact-launch preparation | A test-only D3 broker model now enforces the repository ID/publisher/version tuple, exact expiring capability/decision/session/capsule scope, application-owned active-capsule registration, monotonic rebind and operation IDs, replay rejection, non-wrapping leases, fixed absolute OpenSSH locations, fail-closed configured overrides, native file identity/revalidation, literal bounded argv, zero extension access to inherited environment/secrets, authorization-owned cwd fallback, bounded lifecycle state, revocation, and redacted audit records. The production graph excludes the broker and no managed process/PTY/network path is enabled. ADR acceptance, real package digest/signature/compatibility/revocation proof, visible capability UI, atomic native check-to-spawn, real native lifecycle evidence, D4-to-D5 activation integration, and controlled process/PTY/renderer performance/leak gates remain open. |
| D4 OpenSSH inventory foundation | A private disabled-by-default package now provides exact canonical grants, bounded static concrete-alias parsing, lexical includes, symlink and Unix ownership/permission checks, redacted diagnostics, strict public-only metadata, durable atomic Unix 0700/0600 and protected current-user Windows storage, exact-file watching, burst coalescing, periodic reconciliation, obsolete-generation rejection, and last-known-good recovery. It has only filesystem.read authority and no renderer, PTY, process, launch, network, clipboard, or secret custody. Windows and Linux focused gates passed; a WSL-native libFuzzer run completed 20,863 executions in 11 seconds with no finding at 229 MiB peak RSS, and the Windows optimized 10,000-alias benchmark measured 19.201-19.633 ms. D3 activation and D5 managed connections remain blocked. |
...
4. The repository is currently private. Public visibility, private
   ...
   review counts, CODEOWNERS enforcement, and administrator applicability need
   repository-owner configuration. Authenticated inspection of commit
