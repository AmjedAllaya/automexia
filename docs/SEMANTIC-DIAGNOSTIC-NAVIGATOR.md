...
| Shell command result metadata | Partially implemented reusable foundation | OSC 133 command end records exit code and duration on an identified prompt row for supported integrations; stock CMD remains neutral. | The renderer currently infers visible output bounds, and the command-result surface still has a recorded long-output/scrollback gap. There is no durable complete command-region identity. |
