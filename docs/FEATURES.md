...
| Direct OpenSSH review and guarded launch boundary (nonactivated M2/M3) | Exact alias/literal preparation; `L` host editor; actionable allow-once/session/deny review; one fail-closed application runner; exact executable guard; and ContextManager-owned PTY/route publication seam. Activation is false, the package is unverified, and no production child can start | PR/source: model, adapter, broker/runner, exact-PTY owner, hostile/redaction, bounds, publication/lifecycle, accessibility, focus/keyboard/pointer, tiny-to-8K geometry, strict-lint, architecture, and mutation tests; protected, attestation, current-executable review, screen-reader, native OpenSSH/cleanup/resource evidence remains external | [Connection Hub and SSH](user-guide/connection-hub-and-ssh.md#what-the-current-managed-ssh-preparation-means), [launch contract](SESSION-LAUNCH-BROKER.md), [testing](TESTING.md#m3-direct-openssh-review-contract) | [ADR 0012](adr/0012-first-party-ssh-and-session-launch-boundary.md), [Architecture](ARCHITECTURE.md#m3-direct-openssh-review-boundary) |
