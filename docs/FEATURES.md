...
| Reviewed OpenSSH routes, trust, and guarded lifecycle (nonactivated M2-M4) | Exact direct/config-jump argv; typed host/user/port; complete host-key/public-identity review; safe command copy; fresh full-review/current-executable binding; ContextManager PTY/route publication; child outcomes; fixed notifications; bounded receipts; and reconnect preparation. Activation/status execution are false, the package is unverified, and no production child can start | PR/source: bounded route/trust/status models and hostile/exact/PQ-preserving argv tests; broker/runner/PTY/outcome/receipt/reconnect/accessibility/geometry/schema-4 mutation checks; protected attestation/activation, actual status/OpenSSH, screen-reader, forced-cleanup/resource evidence remains external | [Connection Hub and SSH](user-guide/connection-hub-and-ssh.md#what-the-current-managed-ssh-preparation-means), [launch contract](SESSION-LAUNCH-BROKER.md), [M3 testing](TESTING.md#m3-direct-openssh-review-contract), [M4 testing](TESTING.md#m4-ssh-routes-and-trust-contract) | [ADR 0012](adr/0012-first-party-ssh-and-session-launch-boundary.md), [Architecture](ARCHITECTURE.md#m3-m4-reviewed-openssh-route-and-trust-boundary) |
...
D5.2's typed direct/config-jump routes, full host-key/public-identity review,
safe copy, approval UI, fail-closed runner, and guarded PTY/route seam are now
implemented locally without production authorization. Protected approvals,
attestation, actual status/SSH execution, F5.3 tunnels, native cleanup/resource
proof, and all D6 provider slices remain planned or external. Its reusable
