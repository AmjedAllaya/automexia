...
| Shell integration and listings | Automatic PowerShell, CMD, WSL, Bash, and Zsh prompt provisioning; semantic OSC metadata; icon/category-aware listings without changing piped objects | PR: native hosts | [Shell integration](SHELL-INTEGRATION.md) | [ADR 0009](adr/0009-launch-time-shell-provisioning.md) |
...
| Command completion and persistent Quick Actions | **CP0 architecture only:** accepted native-shell ownership, compatibility/threat fixtures, limits, and non-activation ratchets. CP1+ adapters, action store, aliases, and packs are not shipped | PR: CP0 policy/mutation/architecture/shell regression; planned native runtime gates per CP phase | [Command Productivity](COMMAND-PRODUCTIVITY.md), [compatibility](COMMAND-PRODUCTIVITY-COMPATIBILITY.md), [threat model](COMMAND-PRODUCTIVITY-THREAT-MODEL.md) | [ADR 0015](adr/0015-shell-native-completion-and-typed-quick-actions.md) |
