...
| Native command completion and future Quick Actions | **CP1 shipped:** native-owned PowerShell/Bash/Zsh/Fish adapters, CMD fallback, read-only doctor, explicit bounded official-provider cache, native-definition precedence, disable, and exact uninstall. CP2 action store/CP3 aliases are not shipped. CP5 local suggestions and pane-owned UI are fully planned but remain non-activated behind a separate bridge/threat/dependency gate and native fallback | PR: CP0/CP1 policy mutations, Rust lifecycle/security tests, native shell/install tests on Windows/Linux/macOS; planned CP2/CP3 model/persistence/projection/native-shell/security/performance gates; planned CP5 protocol/fuzz/accessibility/performance/resource/native-shell gates | [Command Productivity](COMMAND-PRODUCTIVITY.md), [DevOps aliases](DEVOPS-ALIASES.md), [compatibility](COMMAND-PRODUCTIVITY-COMPATIBILITY.md), [shell integration](SHELL-INTEGRATION.md), [threat model](COMMAND-PRODUCTIVITY-THREAT-MODEL.md) | [ADR 0015](adr/0015-shell-native-completion-and-typed-quick-actions.md) plus a required future CP5 bridge ADR |
