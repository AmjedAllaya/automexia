...
| Direct OpenSSH review and guarded lifecycle (nonactivated M2/M3) | Exact 17-option-plus-destination requests; fresh full-review/current-executable binding; ContextManager PTY/route publication; actual child outcome; fixed notifications; 256-record/2-MiB private receipts; and stale-source reconnect preparation. Activation is false, the package is unverified, and no production child can start | PR/source: model, exact/PQ-preserving argv, executable replacement, broker/runner, PTY owner, outcome/cancel/shutdown, receipt recovery/bounds/redaction/saturation/restart, reconnect invalidation, accessibility, strict-lint, architecture, assurance and mutation tests; protected attestation/activation, screen-reader, native OpenSSH/forced-cleanup/resource evidence remains external | [Connection Hub and SSH](user-guide/connection-hub-and-ssh.md#what-the-current-managed-ssh-preparation-means), [launch contract](SESSION-LAUNCH-BROKER.md), [testing](TESTING.md#m3-direct-openssh-review-contract) | [ADR 0012](adr/0012-first-party-ssh-and-session-launch-boundary.md), [Architecture](ARCHITECTURE.md#m3-direct-openssh-review-boundary) |
...
D5.2's approval UI, fail-closed runner, and guarded PTY/route seam are now
implemented locally without production authorization. Protected approvals,
attestation, current-executable review binding, native cleanup/OpenSSH/resource
