...
| Native completion and Quick Actions | **CP1 and CP2.2 implemented locally:** shell-owned completion for PowerShell/Bash/Zsh/Fish, truthful CMD fallback, explicit bounded provider cache, typed private Quick Actions, joined latest-only search worker, layered scope index, reviewed placeholders/risk/conflicts, dry-run administration/import/export, and explicit insert/copy without Enter. Workspace actions, secrets, exact launch, aliases, and CP5 suggestions remain disabled | PR: model/hostile/quoting/Unicode/transfer/CAS/recovery/worker/palette/responsive/policy/mutation tests and Criterion search/expansion targets; release: hosted native shell insertion plus controlled AT and 30-day resource evidence remain | [Command Productivity](COMMAND-PRODUCTIVITY.md), [DevOps aliases](DEVOPS-ALIASES.md), [compatibility](COMMAND-PRODUCTIVITY-COMPATIBILITY.md), [shell integration](SHELL-INTEGRATION.md), [threat model](COMMAND-PRODUCTIVITY-THREAT-MODEL.md) | [ADR 0015](adr/0015-shell-native-completion-and-typed-quick-actions.md) plus a required future CP5 bridge ADR |
...
generated DevOps aliases, trusted-workspace actions, secret expansion, or exact
Quick Action launch. CP2.2 is locally available as a reviewed insert/copy
surface; its stable release claim remains gated by native and controlled
evidence.
