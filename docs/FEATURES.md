...
| Native command completion and future Quick Actions | **CP1 shipped; CP2.0 groundwork implemented:** native-owned PowerShell/Bash/Zsh/Fish adapters, CMD fallback, read-only doctor, explicit bounded official-provider cache, plus a capability-free typed Quick Action model/parser/validator. CP2.1 persistence, CP2.2 UI, CP3 aliases, and CP5 suggestions are not shipped | PR: CP0/CP1 policy mutations, CP2.0 Rust hostile/limit/round-trip tests and exact capability ratchet, native shell/install tests on Windows/Linux/macOS; planned CP2.1+/CP3 persistence/projection/native-shell/security/performance gates; planned CP5 protocol/fuzz/accessibility/performance/resource/native-shell gates | [Command Productivity](COMMAND-PRODUCTIVITY.md), [DevOps aliases](DEVOPS-ALIASES.md), [compatibility](COMMAND-PRODUCTIVITY-COMPATIBILITY.md), [shell integration](SHELL-INTEGRATION.md), [threat model](COMMAND-PRODUCTIVITY-THREAT-MODEL.md) | [ADR 0015](adr/0015-shell-native-completion-and-typed-quick-actions.md) plus a required future CP5 bridge ADR |
