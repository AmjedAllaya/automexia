...
| Local and protocol image decoding | Closed locally at the source boundary: quick look requires explicit modifier/action intent, rejects URLs/UNC/symlinks/non-files, caps file/dimensions/pixels/allocation, gates dimensions before full decode, rejects changing files, coalesces per-window generations, and retains only a 16-entry/32 MiB cache with shared renderer pixels. iTerm2 validates base64/declared size and bounded decode dimensions/allocation; Kitty/Sixel keep existing limits and the shared graphics quota. | Retain malformed/compressed-image fuzz and sanitizer evidence plus native Windows/Linux/macOS memory/visual runs before stable release. Third-party image clients and multiplexers remain compatibility inputs, not trusted producers. |
