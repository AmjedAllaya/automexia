...
| Configuration, theme, and legacy migration input | Closed locally: normal loads and migration use bounded UTF-8 readers without panic-prone file reads. Config files are capped at 4 MiB and themes at 1 MiB each. Migration rejects symlinked/non-regular config, caps theme recursion to depth 8, 256 files, 2,048 entries, and 16 MiB aggregate, and caps imported extension activation state to 256 directories, 1,024 entries, 64 KiB per marker, and 1 MiB aggregate. Atomic writes plus interrupted/repeated/oversized/invalid-UTF-8/deep-tree tests keep partial storage bounded. | Retain native Linux/macOS execution of the Unix symlink regression and the recurring config-migration fuzz target. Review the limits when the public config or theme schema gains a legitimate larger payload. |
| Local and protocol image decoding | Closed locally at the source boundary: image-looking path discovery is IO-free; stable hover or explicit click/action submits only local work; URLs/UNC/symlinks/non-files are rejected; file/dimension/pixel/allocation limits gate full decode; changing files are rejected; per-window generations coalesce; and only a 16-entry/32 MiB cache retains shared pixels. Every enabled raster codec, transparency, malformed/mutated input, cache replacement, file-handle release, and no-sidecar behavior have deterministic regressions. Repeated native WGPU/CPU open/dismiss cycles require exact active resource counts/bytes and zero route pixels, overlay, texture, queued work, or pending completion after dismissal, with process-growth ceilings and backend pixel-distribution comparison. The pure decoder/tokenizer fuzz target passed 544,609 explicit-nightly/ASan executions through the supported Windows-to-WSL route; current local builds use disposable storage and RSS/input-time caps. iTerm2 validates base64/declared size and bounded decode dimensions/allocation; Kitty/Sixel keep existing limits and the shared graphics quota. | Retain recurring hosted malformed/compressed-image fuzz and sanitizer evidence, extended soak, and controlled Linux/macOS GPU/memory/visual runs before stable release. Third-party image clients and multiplexers remain compatibility inputs, not trusted producers; finite tests cannot prove every decoder input or driver. |
| Hosted assurance not yet observed | Local dependency policy, architecture, identity, provenance, focused security regressions, and feature-gated test surfaces pass. | Require observed hosted static analysis, fuzz, sanitizers, Miri, cross-platform jobs, signed artifacts, SBOMs, checksums, and signed repository-owned release-manifest evidence before stable release. GitHub artifact attestation is an external Enterprise entitlement for this private repository. |
