...
| M6 typed automation and declarative workspaces | **Partially done; review-only source contracts exist and product execution is disabled** | Extend its immutable revisions, fingerprints, review generations, and no-hook rule instead of adding a second automation store. |
