...
| [0032](adr/0032-bounded-semantic-diagnostic-navigation.md) | Bounded semantic diagnostic navigation (proposed) | Add post-v0.4 on-demand failed-command and error-section navigation with route-local bounded state, app-owned detectors, content-free derived anchors, no persistent index, and no ambient extension history. |
...
| [0037](adr/0037-public-binary-release-distribution.md) | Public binary release distribution | Build and sign in the private source owner, publish an exact immutable Linux bundle to a passive public repository with a scoped GitHub App, and activate first-party website routes only after an independent digest-bound live verification. |
