...
This is a retained navigation mirror. The complete canonical index is
[Architecture decision index](../DECISIONS.md), and all current ADRs live under
[the canonical ADR tree](../adr/). Links below target that tree; no decision is
owned independently here.
...
| [0004](../adr/0004-native-persistent-operational-chrome.md) | Native persistent operational chrome (superseded by ADR 0006) | Renderer-owned tabs/context survive output and resize without polluting terminal cells or PTY history. |
...
| [0009](../adr/0009-launch-time-shell-provisioning.md) | Launch-time shell provisioning (superseded by ADR 0017) | A successful launch should deterministically include supported shell features; manual setup is retained only for repair/uninstall. |
| [0010](../adr/0010-ghostty-keyboard-compatibility.md) | Ghostty-compatible profile design (superseded by ADR 0011) | Platform-specific compatibility must be deliberate, collision-tested, and feature-aware instead of blindly copying a table. |
...
| [0012](../adr/0012-first-party-ssh-and-session-launch-boundary.md) | SSH/session launch boundary (accepted; activation gated) | The project owner accepted the narrow first-party boundary. Production code remains hard disabled and the linked package unverified until ADR 0003 exact-head approvals, attestation, hosted gates, and native evidence pass. |
...
| [0023](../adr/0023-typed-automation-and-declarative-workspaces.md) | Typed automation and declarative workspaces (accepted) | Bind recipes, profiles, layouts, restore, and armed broadcast to immutable review-only generations without adding process, PTY, network, credential, or arbitrary remote-code authority. |
...
| [0031](../adr/0031-versioned-hosted-ci-and-repository-protection.md) | Versioned hosted CI and repository protection | Bind workflows, reviews, least-authority Actions, current evidence, and no-bypass rules to one auditable contract. |
| [0032](../adr/0032-bounded-semantic-diagnostic-navigation.md) | Bounded semantic diagnostic navigation (proposed) | Add post-v0.4 on-demand failed-command and error-section navigation with route-local bounded state, app-owned detectors, content-free anchors, and no ambient extension history. |
...
| [0034](../adr/0034-situation-aware-production-operations.md) | Situation-aware production operations boundary (proposed) | Compose separate deterministic investigation, incident and managed-session capabilities over existing owners without a monolithic operations engine, provider logic, credentials, LLM authority or hidden execution in core. The [strict PO0 contract candidate](../SITUATION-AWARE-PRODUCTION-OPERATIONS-CONTRACTS.md) remains non-activating. |
| [0035](../adr/0035-core-domain-and-optional-extension-ownership.md) | Core domain and optional extension ownership | Keep baseline domains in core/application owners and independently disabled authority in extensions. |
| [0036](../adr/0036-application-owned-runtime-user-preferences.md) | Application-owned runtime user preferences | Layer bounded font and appearance overrides over config without putting storage in VT, PTY, input, renderer, or extensions. |
...
Update [Architecture](../developer/architecture.md), the feature assurance ledger, tests, and
