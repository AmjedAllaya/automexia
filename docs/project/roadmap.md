...
| Assurance | v0.4/S1 | **Fully implemented in source** | **Partial / external** | Exact deterministic visual hooks, Windows WGPU/CPU native/resource automation, separate bounded verifier phases, a strict 28-suite policy/validator with five 9,216-frame visual matrices, controlled workflow, and fail-closed stable-tag dependency are complete. Controlled Linux/macOS/named-GPU/elevated/accessibility evidence and approved visual matrices remain. |
| Performance | S2 | **Fully implemented in source; collecting** | **External baseline pending** | Strict bounded comparable evidence, repeated-sample quality, native memory composition, exact source/operator binding, independent activation review, 90-day retention, a semantically mutation-checked protected activation workflow, and direct fail-closed release dependencies exist; delivery still requires 30 reviewed consecutive controlled-runner days. |
| DevOps | D0 | **Partial** | **Partial** | ADR 0012 is accepted; schema 7 ratchets immutable schemas 1-6 with exact M3-M5 routes/tunnels, trust/status, monotonic review IDs, opaque tunnel receipt ownership, lifecycle/reconnect, and schema-2 native-manifest rules. Protected approvals and real native execution remain. |
...
| Multi-environment | M6/F6 | **Partial overall; review/edit source and product surfaces fully implemented locally** | **Blocked on D3/M5 execution evidence** | Accepted ADR 0023; strict duplicate-name-safe ingress; schema-2 persistence with separated migration/recovery; independently revalidated recipe/broadcast reviews; checked clocks and terminal expiry; large-library bounded restore; public preview-first CLI; immutable worker publication; Hub catalog/restore review; dedicated checker/mutations; fuzz/benchmarks; and explicit 28-suite S1 policy pass locally. Managed execution and executed controlled native/resource/accessibility/9,216-frame visual/release evidence remain. |
