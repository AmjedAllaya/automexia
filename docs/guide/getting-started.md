...
not published until the release prerequisites in the repository `RELEASING.md`
are satisfied.
...
Follow [Windows and WSL development](troubleshooting.md) and use a Linux-native
...
[Shell integration](shell-productivity.md). If a check fails, use the focused
...
custom action names are in [Keyboard and input reference](../reference/keyboard.md).
...
Then use [Configuration reference](../reference/configuration.md) to add only the settings
...
Read the repository `CONTRIBUTING.md` before opening a pull request. Every
