...
Status audited: 2026-08-25 against the current CP4 implementation and contracts.
...
| AWS, Azure, GCP exact actions | **Fully done locally, nonactivating** | provider-owned exact observation/session/cluster contributions and tests | Exact execution and real accounts remain external |
| Kubernetes and OpenShift exact actions | **Fully done locally, broker-required** | discoverable exact contributions preserve private-kubeconfig isolation | Exact broker activation and real clusters/plugins remain external |
| Teleport exact actions | **Fully done locally, broker-required** | discoverable exact contribution preserves Teleport environment/agent isolation | Exact broker activation and real Teleport/MFA remain external |
| OpenBao actions | **Not done; blocked external prerequisite** | provider-neutral enum only; proposed ADR 0024 | ADR acceptance, provider implementation, then a separate CP4 contribution |
| Exact execution | **Not part of CP4 authority** | D3/F4 review and application runner remain separately gated | Provider activation, exact environment binding, native evidence, current reviewed approvals |
| Native/release evidence | **Partially done / external prerequisite** | deterministic model, integration, policy, fuzz, and same-host benchmark evidence | Real accounts, official CLIs, clusters, screen readers, controlled resources, Windows/Linux/macOS release fixtures |
...
Official references rechecked for this slice on 2026-08-25:
...
2. The Connection Hub validates and retains one immutable redacted
   `ProviderProductPublication`; credentials and provider-private state are
   never retained.
3. Each provider extension calls its existing exact plan builder and contributes
   a typed action plus a neutral binding to the action core.
4. Opening Quick Actions synchronizes that retained publication to the selected
   route only after exact route/session/capsule-revision checks. Application
   composition builds and validates the complete index before publication.
5. The Quick Action runtime publishes the snapshot for one route only. A newer
   generation atomically replaces the old entry; an identical product is
   idempotent and stale generations fail closed.
6. The existing worker searches cached indexes only. Request IDs and the
   expected provider generation reject obsolete work before publication.
7. Search and review carry structured provider metadata. Immediately before
...
8. Clearing a route, revoking/replacing a capsule, or dropping the runtime
...
The 2026-08-25 Windows same-host Criterion run (100 samples, 16 AWS fixtures,
optimized benchmark profile) measured snapshot construction at
203.37–208.92 µs and cached search at 47.991–49.943 µs. This is local
comparative evidence rather than a release threshold or multi-platform claim;
the run reported high outliers in 3/100 and 9/100 samples respectively.
...
fuzz, and cached-product handoff are fully integrated at the nonactivating
product boundary. The Connection Hub retains the validated capsule behind a
redacted immutable publication. Opening Quick Actions synchronizes it only to
the exactly matching selected route/session/revision, reuses an identical
snapshot idempotently, and clears mismatched or revoked state before search.
The runtime prebuilds and publishes
...
The schema-1 machine contract fixes seven providers, three publication outcomes,
nine decisions, eleven denied authorities, five limits, fifteen source owners,
fourteen evidence files, 25 named regressions, the fuzz target, benchmark, and
documents. The dedicated checker and seven mutation cases reject authority widening, provider
...
mutations and seven CP4 mutations pass locally (the Windows symlink mutation is
skipped because the current host cannot create that link).

On the 2026-08-25 Windows development host, formatting and warning-denied
workspace Clippy passed. The first locked CI-profile nextest attempt did not
reach the test run: the build volume had about 4 MB free, `link.exe` exited with
`0xc0000005`, and Rust also reported `no space on device`. After removing only
verified reproducible build outputs from the repository target directory, the
clean retry compiled from scratch and ran 2,113 tests successfully, with seven
profile/platform skips and two 141-second keybinding-manifest tests. Locked
workspace documentation tests ran 64 successfully with three upstream
windowing examples ignored.

After the final SSH product-reachability regression, full QA passed every
locally runnable stage, including its independent format, Clippy, nextest,
documentation, repository/PowerShell, dependency, resize-stress, session-clone,
and loom gates. Its CI-profile stage ran 2,114 tests successfully with seven
profile/platform skips. `cargo ready` passed the policy preflight, a clean
non-incremental isolated workspace check/Clippy/test and documentation sequence,
dependency audit, persistent application build, and the `automexia 0.4.0`
executable smoke test; it removed 8.88 GiB of isolated verification artifacts
after success. The initial storage failure remains part of the evidence and is
not represented as a passing run.
...
opt-in and was skipped. No approved provider refresh/capsule producer, exact
provider execution, OpenBao adapter, real account/CLI/cluster, packaging, signing, or
