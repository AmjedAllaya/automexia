...
| DevOps | D3 | **Partial; nonactivated** | **Blocked** | A test-only exact-argv model and hardened local two-independent-human review policy exist; external reviewers/server-side enforcement, real attestation, production launch, capability UX, atomic spawn, and native lifecycle proof do not. |
