...
fn context(
    ...
    ProviderContextTemplate {
        provider,
        configuration_reference: automexia_devops::connections::OpaqueReference::new(
            configuration,
        ),
        ...
        provenance: ProviderContextProvenance {
            ...
            source_reference: automexia_devops::connections::OpaqueReference::new(
                "provider-config.dev",
            ),
...
fn provider_auth_state_machine_covers_refresh_browser_device_and_mfa_waits() {
    ...
    assert_eq!(
        automexia_devops::connections::apply_auth_event(
            available.clone(),
            AuthEvent::MarkStale,
        )
        .unwrap(),
    ...
    let refreshing = automexia_devops::connections::apply_auth_event(
        available.clone(),
    ...
    let authenticating = automexia_devops::connections::apply_auth_event(
        available,
    ...
    .unwrap();
    let browser = automexia_devops::connections::apply_auth_event(
        authenticating,
    ...
    .unwrap();
    let device = automexia_devops::connections::apply_auth_event(
        browser,
    ...
    .unwrap();
    let mfa = automexia_devops::connections::apply_auth_event(
        device,
...
fn global_context_mutations_secret_flags_and_unapproved_origins_fail_closed() {
    ...
    mismatched_isolation.isolation.configuration_reference =
        automexia_devops::connections::OpaqueReference::new("config.other");
