...
Copy-Item (Join-Path $PackageRoot 'shell-integration\powershell\automexia.ps1') $PowerShellIntegration -Force
...
if (-not $SkipWsl -and (Get-Command wsl.exe -ErrorAction SilentlyContinue)) {
    ...
    $bashPath = Join-Path $PackageRoot 'shell-integration\bash\automexia.bash'
    $zshPath = Join-Path $PackageRoot 'shell-integration\zsh\automexia.zsh'
