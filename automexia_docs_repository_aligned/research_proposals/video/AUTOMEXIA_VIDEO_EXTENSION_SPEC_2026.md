...
### 1.1 Explicit local-only intelligence policy
...
### 2.3 CLI-complete; renderer-native review UI optional
...
network = false
gpu = "optional"
filesystem = ["project", "cache", "temp", "explicit-input", "explicit-output"]
managed_tools = ["ffmpeg", "ffprobe"]
...
```text
dedicated inherited pipe/handle + bounded length-prefixed control messages
```

A JSONL stdio protocol is acceptable only as an early prototype; production must not depend on stdout remaining free of native-library diagnostics.
...
```text
Candidate components to evaluate after a video milestone is accepted
├── Rust worker
├── Automexia ProcessBroker integration
├── selected verified FFmpeg / ffprobe runtime after distribution review
├── serde / JSON
├── ONNX Runtime CPU backend
├── Silero VAD
├── FaceDetector abstraction
│   ├── YuNet ONNX — primary 2026 candidate
│   └── UltraFace — benchmark/legacy candidate
├── SpeechEnhancer abstraction
│   ├── deterministic FFmpeg DSP — always-available baseline
│   ├── WebRTC AudioProcessing — active/reference candidate
│   ├── RNNoise — optional lightweight/legacy candidate
│   └── DeepFilterNet — quality candidate pending exact weight-license approval
...
## 18. ML Inference Runtime Candidate

Evaluate **CPU-only ONNX Runtime** only after an accepted feature proves a model and runtime are necessary.
...
The decision engine must depend on a normalized `FaceDetector` interface rather than on one model.

Primary 2026 candidate: **YuNet ONNX**. UltraFace remains a benchmark/legacy candidate until Automexia's own regression corpus proves whether it offers a meaningful speed/quality advantage.

Inference should run on low-resolution display-oriented frames, not full-resolution source frames.

### Speech enhancement abstraction

Automexia must not hard-code RNNoise as the mandatory denoiser.

Use a replaceable `SpeechEnhancer` interface. Rust first decides whether enhancement is needed; clean audio may bypass denoising entirely.

Candidates:

- deterministic FFmpeg DSP — always-available conservative baseline;
- WebRTC AudioProcessing — active low-cost/reference backend;
- RNNoise — optional lightweight/legacy backend if corpus testing justifies it;
- DeepFilterNet — high-quality candidate after exact model-weight redistribution rights and corpus quality are approved.

The architecture depends on the interface and evidence, not on a single model/library.
...
The `voice-quality` pack installs one or more approved higher-quality `SpeechEnhancer` backends. DeepFilterNet is a primary candidate only after exact pretrained-weight redistribution rights and corpus quality are approved.
...
Candidate source-code repositories may use permissive licenses, but release approval must verify the exact code version and every shipped model asset independently. In particular, source-code licensing never implies that pretrained model weights are redistributable under the same terms.
...
Two distribution strategies remain candidates and require a real future video/distribution decision:

```text
A. Automexia-managed verified FFmpeg runtime
B. explicitly approved/probed system/package-manager FFmpeg
```

A managed runtime gives the strongest reproducibility, but it also creates codec, license, redistribution, security-update, signing, packaging, and platform-support obligations. Therefore it is a **preferred reproducibility candidate, not an already approved distribution policy**.

Whichever strategy is selected, runtime identity and capability probing are mandatory. A missing/unsupported runtime produces an actionable dependency/capability error rather than an unsafe fallback.
...
If a managed runtime is approved later, it belongs with the video extension/distribution layer, never the terminal core.

---

## 23. FFmpeg Invocation Rules

Rust compiles media intent; Automexia authorizes; FFmpeg executes.

The video worker must not directly become an arbitrary process-launch authority.

```text
Video worker
    -> typed MediaProcessPlan
    -> Automexia ProcessBroker
    -> verified FFmpeg/ffprobe identity
    -> sandbox/resource policy
```

Never invoke FFmpeg through shell text.

For complex graphs, generate an Automexia-owned temporary filter script and pass its path as a structured argument. The ProcessBroker owns executable identity, environment sanitization, cancellation, child-tree cleanup, and redacted receipts.
...
```json
{
  ...
  "hardware":["..."],
  "onnx_provider":"cpu",
  "models":["silero-vad","yunet-candidate"],
  "features":["base"]
}
```
...
decode / channel selection
    ...
    -> optional approved SpeechEnhancer chosen by deterministic policy
    -> edit/cut assembly
    -> conservative EQ
    -> de-esser
    -> compression
...
Use FFmpeg filters where they are mature and appropriate; an approved `SpeechEnhancer` can be integrated as a replaceable native processing stage when analysis predicts benefit.
...
1. run the approved `FaceDetector` (YuNet is the primary 2026 candidate) on sampled low-resolution frames
...
selected source audio
    -> source-level cleanup / approved SpeechEnhancer where enabled
...
- SpeechEnhancer processing that the target cannot represent -> rendered audio stem
...
runtime = "onnx"
sha256 = "..."
input_width = "dynamic-or-reviewed-profile"
input_height = "dynamic-or-reviewed-profile"
license = "review-exact-model"
```
...
- conservative approved speech enhancement only when analysis predicts benefit
...
- approved SpeechEnhancer only when analysis predicts benefit
...
- approved `FaceDetector` (YuNet primary candidate after corpus qualification)
- light face tracking
...
- approved higher-quality SpeechEnhancer pack (DeepFilterNet candidate after model-license approval)
...
8. run approved FaceDetector (YuNet primary candidate) + lightweight track association
...
- versioned bounded dedicated worker IPC
...
- SpeechEnhancer/voice chain
- scene detection
- FaceDetector abstraction / YuNet qualification
- basic reframe
- render compiler
...
>
> **Models observe. Rust decides. Automexia authorizes. FFmpeg executes. Humans review uncertainty.**
...
## 105. Normative 2026 Architecture Amendments

This research addendum supersedes conflicting wording only inside this proposal; it is not normative project documentation.

The purpose is to explore how `automexia-video` could fit the current Rust-first extension and overlay seams, accepted capability policy, and the existing application-owned external-tool boundary while preserving the original product goal.

The permanent mental model is now:
...
## 115. Denoiser Bake-Off and Quality Gate

Before selecting the default SpeechEnhancer, create a dedicated audio corpus:
...
Also run blind human listening.

The default is selected by evidence, not dependency popularity.

---

## 116. Audio Enhancement Pipeline

Audio preflight determines whether cleanup is necessary.
...
preflight
  ...
  ├── speech occupancy
  ├── noise/SNR
  └── continuous music/non-speech bed
        ↓
Rust EnhancementDecision
        ↓
optional SpeechEnhancer
        ↓
timeline assembly
        ↓
conservative EQ / de-ess / compression
        ↓
loudness normalization
        ↓
true-peak limiting
...
Do not denoise already-clean audio.

Do not aggressively denoise a mixed music bed by default.
...
Add a dedicated Automexia Lab module:
...
VideoLab runs deterministic and native scenarios across:
...
```yaml
scenario: talking-head-natural-pause
input: fixtures/talking-head-01.mov
preset: clean
...
## 124. VideoLab Failure and Hostile-Media Harnesses
...
false duration metadata
zero/negative timestamps
broken stream tables
```
...
memory/process/temp usage remains bounded
.amxv is not corrupted
incomplete output is handled predictably
valid analysis may be reused on retry
```

---

## 125. Media Regression Corpus

Build the corpus before heavy tuning.

Include legally usable samples for:
...
Each release compares edit decisions and output properties against accepted baselines.
...
noise attenuation
speech damage
blind listening preference
```

Color:

```text
auto-correction rejection rate
clipping change
shot consistency
```

One of the most important product metrics is:

```text
percentage of automated edits users keep
```

This directly measures trustworthiness.
...
## 129. Revised Final Architecture
...
```text
          ...
          ┌─────────────────────┼─────────────────────┐
          ▼                     ▼                     ▼
   Media Foundation       Analysis Layer          Export Layer
          │                     │                     │
 ffprobe/media model      Silero VAD              OTIO JSON
 rational time            FaceDetector            FCP7 XML
 color/HDR capability     Audio analysis          EDL
          │                     │
          └────────────┬────────┘
                       ▼
                Observation Cache
