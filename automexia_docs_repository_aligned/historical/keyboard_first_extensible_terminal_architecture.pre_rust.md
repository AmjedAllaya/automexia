# Keyboard-First Extensible Terminal Platform
...
**Primary goal:** Define a robust architecture for a fast terminal emulator that is useful by itself, but can become a broader terminal platform through secure extensions and add-ons.
...
2. A **secure capability platform** that exposes commands, resources, sessions, authentication, process execution, storage, and terminal-native overlays.
...
│                           Terminal UI                                │
│                                                                      │
│  GPU Renderer   Keyboard Input   Command Palette   Terminal Overlays │
└───────────────────────────────┬──────────────────────────────────────┘
                                │
                              IPC/API
                                │
...
## 3.2 Keyboard-first, not GUI-first

The product should avoid becoming a settings-dashboard application.
...
- command palette,
- fuzzy search,
- terminal-native pickers,
- terminal-native confirmations,
- inline/overlay progress,
- structured terminal-native lists and tables,
- searchable command/resource actions.

“Without GUI” should not mean “without user interface.”

It should mean:

> no dependency on complex traditional windowed management interfaces.

The terminal itself can render lightweight overlays when user interaction is necessary.
...
The PTY and shell session should not necessarily die just because the UI process dies.
...
# 18. Terminal-Native Overlay System

A keyboard-first terminal still needs structured interaction.

The trusted core should provide declarative UI primitives.
...
- one theme system,
- accessibility,
- no extension-specific UI toolkit,
...
# 19. Overlay Rendering

Overlays should be composited separately:
...
The resource picker searches the local index instantly.
...
[keybindings]
"ctrl+space" = "commandPalette.open"
...
- show destination on hover/preview if visual UI permits,
...
- UI crash → session survives if architecture permits,
...
The UI thread should do very little.
...
# 94. Recommended Language and Runtime

A strong implementation choice is:

```text
Rust for the application/control plane
```

because the product includes:
...
├── wgpu or custom native GPU abstraction
├── Wasmtime
└── wrapped terminal/VT engine
```

This is a recommendation, not an absolute requirement.

Zig is also excellent for low-level terminal work.

The architecture matters more than language loyalty.
...
This allows generic resource UI.
...
> UI restart does not unnecessarily destroy healthy sessions.

---

## Phase 3 — Command and Overlay Platform
...
> Most platform functionality can be invoked without adding custom UI code.
...
This should be accessible through command palette or `terminal doctor`.

---

# 131. Authentication UI

Authentication interaction should be handled through trusted core UI surfaces.
...
- a low-friction keyboard-native experience,
