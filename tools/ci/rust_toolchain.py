...
def _validate_policy(root: Path) -> int:
    ...
    for name in RUST_WORKFLOWS:
        ...
        for job_number, (job_id, job) in enumerate(workflow["jobs"].items(), 1):
            ...
            if name in {"ci.yml", "linux-early-access.yml"} and job_id == "quality":
                initializer = r'''printf 'SCCACHE_GHA_VERSION=automexia-rust-%s-v2\n' "$RUSTUP_TOOLCHAIN" >> "$GITHUB_ENV"'''
                initializers = [index for index, step in enumerate(steps)
                                if initializer in step.get("run", "")]
                cache_steps = [index for index, step in enumerate(steps)
                               if step.get("id") == "sccache"]
                if len(initializers) != 1 or len(cache_steps) != 1 or initializers[0] >= cache_steps[0]:
                    raise ToolchainError(f"{location}: compiler cache initializer missing or reordered")
                lines = [line.strip() for line in steps[initializers[0]]["run"].splitlines()
                         if "SCCACHE_GHA_VERSION" in line]
                if lines != [initializer]:
                    raise ToolchainError(f"{location}: compiler cache initializer changed")
