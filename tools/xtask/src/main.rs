...
mod tests {
    ...
    fn summarized_command_returns_bounded_success_output() {
        ...
        let output =
            run_bounded_capture(command, std::time::Duration::from_secs(5), 4096)
    ...
    fn summarized_command_deadline_terminates_the_owned_child() {
        ...
        let error =
            run_bounded_capture(command, std::time::Duration::from_millis(100), 4096)
    ...
    fn summarized_command_output_limit_fails_closed() {
        ...
        let error = run_bounded_capture(command, std::time::Duration::from_secs(5), 512)
    ...
    fn bounded_runner_child() {
        match env::var("AUTOMEXIA_XTASK_TEST_CHILD_MODE").as_deref() {
            ...
            Ok("overflow") => {
                println!("bounded-child-overflow\n{}", "x".repeat(4096));
            }
            Ok("wait") => {
