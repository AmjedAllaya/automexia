...
#[cfg(not(target_arch = "wasm32"))]
fn root_dir() -> PathBuf {
    // Namespaced state keeps Automexia-owned data separate while the 0.x line
    // still uses Rio's config root. A standalone Automexia config root can be
    // substituted later without touching the extension runtime contract.
...
#[cfg(not(target_arch = "wasm32"))]
pub fn set_installed(manifest: &ExtensionManifest, installed: bool) -> Result<(), String> {
    if installed {
...
#[cfg(target_arch = "wasm32")]
pub fn set_installed(_manifest: &ExtensionManifest, _installed: bool) -> Result<(), String> {
    Ok(())
}
...
fn atomic_write(path: &Path, contents: &str) -> Result<(), String> {
    ...
    let file_name = path.file_name().and_then(|name| name.to_str()).unwrap_or("marker");
