...
fn detect_native(session: &SessionFacts) -> DevOpsSnapshot {
    ...
    let kubernetes = kubernetes_context(home, view.use_process_env).or_else(|| {
        ...
        project_context
            ...
            .or_else(|| legacy_context.as_ref().and_then(kubernetes_from_automexia_json))
    });
    ...
    let docker = docker_context(home, view.use_process_env)
        ...
        .or_else(|| project_context.as_ref().and_then(docker_from_automexia_json))
    ...
    if view.wsl.is_some() {
        for host_cloud in cloud_contexts(host_home.as_deref(), None, None, true) {
            if !clouds.iter().any(|candidate| candidate.provider == host_cloud.provider) {
    ...
    let terraform = terraform_workspace(cwd, view.use_process_env)
        .or_else(|| project_context.as_ref().and_then(terraform_from_automexia_json))
        .or_else(|| legacy_context.as_ref().and_then(terraform_from_automexia_json));
    ...
    let environment = view
        ...
        .flatten()
        .or_else(|| project_context.as_ref().and_then(environment_from_automexia_json))
        .or_else(|| legacy_context.as_ref().and_then(environment_from_automexia_json));

    let project = project_context
        .as_ref()
        .and_then(project_from_automexia_json)
        .or_else(|| legacy_context.as_ref().and_then(project_from_automexia_json));
    ...
    let production = kubernetes
        ...
        .chain(project.iter())
        .chain(clouds.iter().flat_map(|cloud| [&cloud.profile, &cloud.region]))
...
/// `amjed@<REDACTED_LOCAL_VALUE>:/mnt/d/work` or `amjed@host: /mnt/d/work`.
...
fn windows_wsl_session_view(session: &SessionFacts) -> Option<SessionView> {
    ...
    let distro_label = session
        ...
        .or_else(|| session.distro.as_ref().filter(|value| !value.trim().is_empty()))
        .cloned()
...
#[cfg(not(target_arch = "wasm32"))]
fn kubernetes_context(home: Option<&Path>, use_process_env: bool) -> Option<KubernetesContext> {
    let paths: Vec<PathBuf> = match use_process_env.then(|| env::var_os("KUBECONFIG")).flatten() {
        Some(value) => env::split_paths(&value).take(MAX_KUBECONFIG_FILES).collect(),
        None => home
    ...
    let documents: Vec<String> = paths.iter().filter_map(|path| read_small_text(path)).collect();
...
fn kube_namespace_for_context(content: &str, wanted: &str) -> Option<String> {
    ...
    let flush = |name: &mut Option<String>, namespace: &mut Option<String>| -> Option<String> {
...
fn aws_context(home: Option<&Path>, use_process_env: bool) -> Option<CloudContext> {
    let explicit_profile = use_process_env
        .then(|| env::var("AWS_PROFILE").ok().or_else(|| env::var("AWS_DEFAULT_PROFILE").ok()))
        .flatten()
        .filter(|value| !value.trim().is_empty());
    ...
    let region = use_process_env
        .then(|| env::var("AWS_REGION").ok().or_else(|| env::var("AWS_DEFAULT_REGION").ok()))
        .flatten()
        .filter(|value| !value.trim().is_empty())
...
fn azure_context(home: Option<&Path>, use_process_env: bool) -> Option<CloudContext> {
    if let Some(subscription) = use_process_env
        .then(|| env::var("AZURE_SUBSCRIPTION").ok().or_else(|| env::var("AZURE_SUBSCRIPTION_ID").ok()))
        .flatten()
        .filter(|value| !value.trim().is_empty())
    {
...
fn gcp_context(home: Option<&Path>, use_process_env: bool) -> Option<CloudContext> {
    if let Some(project) = use_process_env
        .then(|| env::var("GOOGLE_CLOUD_PROJECT").ok().or_else(|| env::var("CLOUDSDK_CORE_PROJECT").ok()))
        .flatten()
        .filter(|value| !value.trim().is_empty())
    {
        return Some(CloudContext {
...
fn legacy_automexia_context(home: Option<&Path>, use_process_env: bool) -> Option<Value> {
    let root = use_process_env
        .then(|| env::var_os("XDG_CONFIG_HOME").map(PathBuf::from)
...
fn kubernetes_from_automexia_json(value: &Value) -> Option<KubernetesContext> {
    let nested = value
        ...
        .unwrap_or(value);
    let context = json_string(nested, &["kubernetesContext", "kubernetes_context", "context"])?;
    let namespace = json_string(nested, &["namespace"]).unwrap_or_else(|| "default".to_string());
    Some(KubernetesContext {
...
fn terraform_from_automexia_json(value: &Value) -> Option<String> {
    ...
    json_string(nested, &["terraformWorkspace", "terraform_workspace", "workspace"])
}
...
fn cloud_from_automexia_json(value: &Value) -> Option<CloudContext> {
    ...
    let provider_raw = json_string(nested, &["cloudProvider", "cloud_provider", "provider"])?;
...
fn looks_production(value: &str) -> bool {
    value
        ...
        .any(|token| matches!(token.to_ascii_lowercase().as_str(), "prod" | "production" | "live"))
}
...
mod tests {
    ...
    fn wsl_title_parser_accepts_common_bash_shapes() {
        assert_eq!(
            parse_wsl_title("amjed@<REDACTED_LOCAL_VALUE>:/mnt/d/work"),
            Some(("amjed".to_string(), "/mnt/d/work".to_string()))
        );
        assert_eq!(
            parse_wsl_title("amjed@<REDACTED_LOCAL_VALUE>: /home/amjed/project"),
    ...
    fn existing_docker_config_root_implies_default_context() {
        ...
        assert_eq!(docker_context(Some(&home), false).as_deref(), Some("default"));
    ...
    fn nested_project_context_fields_are_supported() {
        ...
        .unwrap();
        assert_eq!(docker_from_automexia_json(&value).as_deref(), Some("desktop-linux"));
        assert_eq!(terraform_from_automexia_json(&value).as_deref(), Some("staging"));
        assert_eq!(git_from_automexia_json(&value).as_deref(), Some("main"));
        assert_eq!(project_from_automexia_json(&value).as_deref(), Some("payments"));
        assert_eq!(environment_from_automexia_json(&value).as_deref(), Some("staging"));
        assert_eq!(kubernetes_from_automexia_json(&value).unwrap().namespace, "payments");
