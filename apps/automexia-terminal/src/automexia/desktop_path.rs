//! Shared explicit desktop path resolution. Callers choose the filesystem kind.
...
fn invalid() -> io::Error {
    io::Error::new(io::ErrorKind::InvalidInput,
        "provide an existing path of the requested type, at most 4096 UTF-8 bytes and no control characters")
}
...
pub(super) fn resolve(
    ...
    if session.has_guest_hints() {
        let result = local_tools::run_tool(kind.probe(), &[input.into()], session)?;
        ...
        resolve_native(Path::new(input), kind)
    }
}

fn resolve_native(path: &Path, kind: Kind) -> io::Result<String> {
...
fn guest_destination(distribution: &str, path: &str) -> io::Result<String> {
        ...
        || (path != "/" && !path[1..].split('/').all(windows_directory_component))
    {
        return Err(io::Error::new(io::ErrorKind::InvalidInput,
            "Linux path cannot be represented safely by the Windows desktop; use a native Linux tool for this path"));
    }
    Ok(format!(
...
mod tests {
    ...
    fn resolve(input: &str, session: &ToolSession) -> io::Result<String> {
        super::resolve(input, session, Kind::Directory)
    }
    fn resolve_native(path: &Path) -> io::Result<String> {
        super::resolve_native(path, Kind::Directory)
    }
