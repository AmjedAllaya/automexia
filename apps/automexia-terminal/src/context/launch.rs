...
mod tests {
    ...
    fn configured_wsl_descriptor_is_an_exact_early_fallback() {
        let source = descriptor(
            ...
            &[
                ...
                "--cd",
                "/home/<REDACTED_LOCAL_VALUE>/project",
                "--exec",
                "/bin/zsh",
                "-l",
            ],
    ...
    fn configured_wsl_clone_overlays_live_identity_and_directory() {
        ...
        let clone = source
            .fresh_clone(&LiveSessionMetadata {
                ...
                user: Some("<REDACTED_LOCAL_VALUE>".to_string()),
    ...
    fn invalid_live_wsl_directory_uses_the_explicit_profile_directory() {
        let source = descriptor(
            ...
            &[
                ...
                "<REDACTED_LOCAL_VALUE>",
                "--cd",
                "/home/<REDACTED_LOCAL_VALUE>/safe",
                "--exec",
                "/bin/zsh",
                "-l",
            ],
        ...
        assert!(clone
            .args()
            .windows(2)
            .any(|pair| pair == ["--cd", "/home/<REDACTED_LOCAL_VALUE>/safe"]));
    }
    ...
    fn nested_wsl_is_reconstructed_instead_of_falling_back_to_powershell() {
        ...
        let clone = source
            .fresh_clone(&LiveSessionMetadata {
                current_directory: Some(PathBuf::from("/home/<REDACTED_LOCAL_VALUE>/work tree")),
                distro: Some("Ubuntu".to_string()),
                user: Some("<REDACTED_LOCAL_VALUE>".to_string()),
                shell_name: Some("zsh".to_string()),
        ...
        assert_eq!(
            ...
            [
                ...
                "<REDACTED_LOCAL_VALUE>",
                "--cd",
                "/home/<REDACTED_LOCAL_VALUE>/work tree",
                "--exec",
                "/usr/bin/zsh",
                "-l",
            ]
    ...
    fn incomplete_nested_wsl_never_silently_opens_powershell() {
        ...
        let error = source
            .fresh_clone(&LiveSessionMetadata {
                current_directory: Some(PathBuf::from("/home/<REDACTED_LOCAL_VALUE>")),
                distro: Some("Ubuntu".to_string()),
                user: Some("<REDACTED_LOCAL_VALUE>".to_string()),
                ..LiveSessionMetadata::default()
            })
